        ...
        zombie: {
            ...
            _update: function() {
                ...
                if (this.isAdjacent(this.game.player.x, this.game.player.y)) {
                    this.game.resolveAttack(this, this.meleeWeapon, this.game.player);
                    return;
                }
                ...
                if (this.playerLastSeen) {
                    ...
                    // try to find a path through furniture
                    if (!destination) {
                        ...
                        if(destination){
                            var furniture = this.game.furnitureManager.getFirst(destination.x, destination.y, function(furniture){
                                return furniture.destroyable;
                            });

                            if(furniture){
                                this.game.resolveAttack(this, this.meleeWeapon, furniture);
                                return;
                            }

                            var entity = this.game.entityManager.get(destination.x, destination.y);
                            if(entity && entity !== this.game.player){
                                entity.hordePushBonus += this.meleeWeapon.damage + this.hordePushBonus;
                                return;
                            }
                        }
                    }
